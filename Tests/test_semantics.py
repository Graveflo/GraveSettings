...
from grave_settings.semantics import Semantics, Semantic, SemanticContext
...
class TestSemanticContext(TestCase):
    ...
    def test_walk_stack_frame(self):
        s = self.get_semantics()
        with s:
            s.add_frame_semantic(DummyIntSemantic(5))
            with s:
                s.add_frame_semantic(DummyIntSemantic(7))
                with s:
                    s.add_frame_semantic(DummyIntSemantic(20))
                    with s:
                        with s:
                            s.add_frame_semantic(DummyIntSemantic(99))
    ...
    def test_transfers_parent_up_not_down(self):
        ...
        with s:
            ...
            with s:
                s.add_frame_semantic(StackingSemantic(2))
    ...
    def test_semantic_frame_merge_collection(self):
        ...
        with s:
            s.add_frame_semantic(StackingSemantic(3))
            colec = s[StackingSemantic]
            self.assertIn(StackingSemantic(0), colec)
